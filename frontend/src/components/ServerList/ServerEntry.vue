...
<div class="server_entry" :class="{'clicked': getSelectedServer() === ip}" @dblclick="$router.push('/server/' + ip)" @click="changeSelectedServer(ip)" @mouseenter="hovering_global = true" @mouseleave="hovering_global = false">
    <div class="icon_wrapper">
        <img class="server_icon" :class="{'hovering': hovering_global}" :src="icon && icon !== 'None' ? (icon.startsWith('data:') || icon.startsWith('http') || icon.startsWith('/') ? icon : `${API_URL}/static/favicons/${icon[0]}/${icon}.png`) : (props.data.type === 1 ? 'https://minecraft.wiki/images/Bedrock_JE2_BE2.png' : `${API_URL}/static/server/unknown_server.png`)">
...
<style>
/* MOTD select, needs to be unscoped */
